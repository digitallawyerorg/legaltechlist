...
class StatisticsHelperTest < ActiveSupport::TestCase
  ...
  test "stats_chart_neighbors wraps around for technology themes" do
    neighbors = neighbors_for("tag_distribution")

    assert_equal "AI in Legal Tech", neighbors[:prev][:title]
    assert_equal statistics_ai_trends_path, neighbors[:prev][:path]
